...
import "./data-lab.css";
import "./overview.css";
import "./overview-visibility.css";
...
type TrainingSample = { id: string; image_url: string; plate_label?: string | null; status: "labeled" | "unlabeled"; created_at: string };
type ModelStatus = { model_name: string; custom_plate_model: boolean; dataset_total: number; dataset_labeled: number; evaluation: { precision: number | null; recall: number | null; map50: number | null }; latest_confidence?: { plate_confidence?: number | null; face_confidence?: number | null; occurred_at?: string } | null };
...
function App() {
  ...
  const [activeView, setActiveView] = useState<"overview" | "monitor" | "analysis" | "history" | "data">("overview");
  const [samples, setSamples] = useState<TrainingSample[]>([]);
  const [modelStatus, setModelStatus] = useState<ModelStatus | null>(null);
  const [datasetFile, setDatasetFile] = useState<File | null>(null);
  const [datasetPlate, setDatasetPlate] = useState("");
  ...
  const load = async () => {
    try { const [eventResponse, sessionResponse, sampleResponse, modelResponse] = await Promise.all([fetch(`${API}/events`), fetch(`${API}/sessions/recent`), fetch(`${API}/dataset/samples`), fetch(`${API}/model/status`)]); setEvents(await eventResponse.json()); setSessions(await sessionResponse.json()); setSamples(await sampleResponse.json()); setModelStatus(await modelResponse.json()); setStatus("Hệ thống sẵn sàng"); }
  ...
  const addTrainingSample = async (event: FormEvent) => {
    event.preventDefault();
    if (!datasetFile) { setDatasetMessage("Hãy chọn một ảnh xe/biển số trước."); return; }
    const form = new FormData(); form.append("image", datasetFile); form.append("plate_label", datasetPlate);
    try {
      const response = await fetch(`${API}/dataset/samples`, { method: "POST", body: form }); const result = await response.json();
      if (!response.ok) throw new Error(result.detail || "Không thể thêm ảnh dữ liệu");
      setDatasetMessage(result.status === "labeled" ? "Đã thêm ảnh và gán nhãn vào dataset." : "Đã thêm ảnh; hãy chọn ảnh để gán nhãn sau.");
      setDatasetFile(null); setDatasetPlate(""); load();
    } catch (error) { setDatasetMessage(error instanceof Error ? error.message : "Không thể thêm ảnh dữ liệu"); }
  };

  const saveSampleLabel = async (event: FormEvent) => {
    event.preventDefault();
    if (!selectedSampleId || !sampleLabel.trim()) return;
    const form = new FormData(); form.append("plate_label", sampleLabel);
    try {
      const response = await fetch(`${API}/dataset/samples/${selectedSampleId}`, { method: "PATCH", body: form }); const result = await response.json();
      if (!response.ok) throw new Error(result.detail || "Không thể lưu nhãn");
      setDatasetMessage(`Đã gán nhãn ${result.plate_label}.`); setSelectedSampleId(""); setSampleLabel(""); load();
    } catch (error) { setDatasetMessage(error instanceof Error ? error.message : "Không thể lưu nhãn"); }
  };
  ...
  return <main className={`view-${activeView}`}>
    ...
    <nav className="app-nav" aria-label="Chức năng hệ thống"><button className={activeView === "overview" ? "active" : ""} onClick={() => setActiveView("overview")}>Tổng quan</button><button className={activeView === "monitor" ? "active" : ""} onClick={() => setActiveView("monitor")}>Giám sát cổng</button><button className={activeView === "analysis" ? "active" : ""} onClick={() => setActiveView("analysis")}>Phân tích AI</button><button className={activeView === "history" ? "active" : ""} onClick={() => setActiveView("history")}>Nhật ký</button><button className={activeView === "data" ? "active" : ""} onClick={() => setActiveView("data")}>Dữ liệu & Model</button></nav>
    ...
    <section className="cards"><article><span className="card-icon blue">◷</span><div><strong>{todayEvents.length}</strong><span>Lượt kiểm soát hôm nay</span></div></article><article><span className="card-icon green">✓</span><div><strong>{todayEvents.filter(e => e.decision === "approved").length}</strong><span>Được cho phép hôm nay</span></div></article><article><span className="card-icon red">×</span><div><strong>{todayEvents.filter(e => e.decision !== "approved").length}</strong><span>Không cho phép hôm nay</span></div></article></section><section className="overview-peak"><article className="overview-command"><div className="overview-kicker"><span><i /> LIVE CONTROL CENTER</span><time>{currentTime.toLocaleTimeString("vi-VN", { hour: "2-digit", minute: "2-digit", second: "2-digit" })}</time></div><h2>Hệ thống đang <em>{liveCameraOn ? "giám sát trực tiếp" : "chờ kết nối camera"}.</em></h2><p>{liveCameraOn ? "AI đang đồng thời theo dõi khuôn mặt, biển số và điều phối quyết định qua cổng." : "Mở mục Giám sát cổng để kết nối camera và bắt đầu quét tự động."}</p><div className="system-pulse"><span><i className={liveCameraOn ? "ok" : ""} />Camera</span><span><i className={faceScanning ? "ok" : ""} />Face AI</span><span><i className={liveScanning ? "ok" : ""} />Gate AI</span><span><i className={status === "Hệ thống sẵn sàng" ? "ok" : ""} />API</span></div></article><article className="overview-process"><p className="eyebrow">LUỒNG KIỂM SOÁT THÔNG MINH</p><div className="process-steps"><div><b>01</b><span>Camera<br /><small>Thu hình ảnh</small></span></div><i>→</i><div><b>02</b><span>AI Vision<br /><small>Đọc mặt & xe</small></span></div><i>→</i><div><b>03</b><span>Đối chiếu<br /><small>Xác minh lượt</small></span></div><i>→</i><div><b>04</b><span>Barrier<br /><small>Cho phép/Giữ</small></span></div></div><div className="overview-footer"><span>Ngày {currentTime.toLocaleDateString("vi-VN", { weekday: "long", day: "2-digit", month: "2-digit", year: "numeric" })}</span><b>{todayEvents.length ? `${todayEvents.length} lượt đã xử lý hôm nay` : "Sẵn sàng đón lượt xe đầu tiên"}</b></div></article></section>
    ...
    <section className="data-lab"><div className="data-lab-head"><div><p className="eyebrow">DATASET LAB & MODEL EVALUATION</p><h2>Chuẩn bị dữ liệu training</h2><p>Thu ảnh thật, gán nhãn biển số và đánh giá model một cách minh bạch.</p></div><span>{modelStatus?.custom_plate_model ? "MODEL BIỂN SỐ RIÊNG" : "YOLO TỔNG QUÁT"}</span></div><div className="readiness-grid"><article><small>MODEL ĐANG DÙNG</small><b>{modelStatus?.model_name || "Đang tải..."}</b><span>{modelStatus?.custom_plate_model ? "Sẵn sàng kiểm thử model riêng" : "Cần training model biển số Việt Nam"}</span></article><article><small>TIẾN ĐỘ DATASET</small><b>{modelStatus?.dataset_labeled || 0} / {modelStatus?.dataset_total || 0} ảnh đã gán nhãn</b><span>{(modelStatus?.dataset_labeled || 0) >= 100 ? "Đủ để thử nghiệm train ban đầu" : "Mục tiêu tối thiểu: 100 ảnh đa điều kiện"}</span></article><article><small>ĐỘ TIN CẬY QUÉT GẦN NHẤT</small><b>{modelStatus?.latest_confidence?.plate_confidence ? `${Math.round(modelStatus.latest_confidence.plate_confidence * 100)}% biển số` : "Chưa có biển số rõ"}</b><span>{modelStatus?.latest_confidence?.face_confidence ? `${Math.round(modelStatus.latest_confidence.face_confidence * 100)}% khuôn mặt` : "Chưa có khuôn mặt được lưu"}</span></article><article><small>SỨC KHỎE CAMERA</small><b className={liveCameraOn ? "health-online" : "health-offline"}>{liveCameraOn ? "● Online" : "● Offline"}</b><span>{liveCameraOn ? `${faceScanning ? "Quét mặt" : "Dừng quét mặt"} · ${liveScanning ? "Quét cổng" : "Dừng quét cổng"}` : "Mở mục Giám sát cổng để kết nối"}</span></article></div><section className="training-workspace"><form className="dataset-upload" onSubmit={addTrainingSample}><h3>Thêm ảnh vào Dataset</h3><p>Dùng ảnh xe/biển số được chụp trong nhiều góc, khoảng cách và ánh sáng khác nhau.</p><label>Ảnh xe hoặc biển số<input type="file" accept="image/*" onChange={e => setDatasetFile(e.target.files?.[0] || null)} /></label><label>Biển số thật <small>(có thể gán sau)</small><input value={datasetPlate} placeholder="Ví dụ: 43-A1 123.45" onChange={e => setDatasetPlate(e.target.value)} /></label><button>Thêm vào Dataset <span>+</span></button>{datasetMessage && <p className="dataset-message">{datasetMessage}</p>}</form><div className="training-guide"><h3>Đánh giá model</h3><div><b>Precision</b><span>{modelStatus?.evaluation.precision === null ? "Chưa đánh giá" : modelStatus?.evaluation.precision}</span></div><div><b>Recall</b><span>{modelStatus?.evaluation.recall === null ? "Chưa đánh giá" : modelStatus?.evaluation.recall}</span></div><div><b>mAP@50</b><span>{modelStatus?.evaluation.map50 === null ? "Chưa đánh giá" : modelStatus?.evaluation.map50}</span></div><p>Chỉ số sẽ xuất hiện sau khi có tập test và chạy đánh giá model đã train.</p></div></section><section className="sample-panel"><div><h3>Ảnh dữ liệu gần đây</h3><span>{samples.length} ảnh</span></div>{samples.length === 0 ? <p className="samples-empty">Chưa có ảnh. Hãy thêm ảnh thật từ camera để bắt đầu tạo dataset.</p> : <div className="sample-grid">{samples.map(sample => <button type="button" className={`sample-card ${sample.status} ${selectedSampleId === sample.id ? "selected" : ""}`} onClick={() => { setSelectedSampleId(sample.id); setSampleLabel(sample.plate_label || ""); }} key={sample.id}><img src={`http://127.0.0.1:8000${sample.image_url}`} alt="Ảnh dataset" /><b>{sample.plate_label || "Chưa gán nhãn"}</b><span>{sample.status === "labeled" ? "Đã gán nhãn" : "Cần gán nhãn"}</span></button>)}</div>}{selectedSampleId && <form className="label-editor" onSubmit={saveSampleLabel}><b>Gán nhãn ảnh đã chọn</b><input value={sampleLabel} placeholder="Nhập biển số thật" onChange={e => setSampleLabel(e.target.value)} autoFocus /><button>Lưu nhãn</button></form>}</section></section>
  </main>;
}
