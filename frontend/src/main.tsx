...
function App() {
  ...
  return <main>
    ...
    <section className="history-panel"><div className="history-head"><div><p className="eyebrow">LỊCH SỬ HỆ THỐNG</p><h2>Nhật ký kiểm soát gần đây</h2><p>Ảnh và kết quả được lưu theo từng lần xe qua cổng.</p></div><div className="history-total"><strong>{events.length}</strong><span>lượt ghi nhận</span></div></div>{events.length === 0 ? <div className="history-empty"><b>Chưa có lịch sử quét</b><span>Khi AI xác minh một lượt xe, kết quả sẽ hiện tại đây.</span></div> : <div className="history-table"><div className="history-columns"><span>Thời gian</span><span>Làn xe</span><span>Biển số</span><span>Kết quả</span><span>Ghi chú & bằng chứng</span></div>{events.slice(0, 12).map(e => <article className="history-row" key={e.id}><time>{new Date(e.occurred_at).toLocaleString("vi-VN")}</time><span className={`direction ${e.direction}`}>{e.direction === "entry" ? "↘ Vào" : "↗ Ra"}</span><b>{e.plate_number || "Chưa đọc được"}</b><span className={`history-status ${e.decision}`}>{e.decision === "approved" ? "✓ Cho phép" : e.decision === "manual_review" ? "Chờ kiểm tra" : "× Từ chối"}</span><div className="history-note">{e.evidence_url ? <img src={`http://127.0.0.1:8000${e.evidence_url}`} alt={`Ảnh quét ${e.plate_number || "xe"}`} /> : <span className="no-evidence">Không có ảnh</span>}<p>{e.reason}</p></div></article>)}</div>} {events.length > 12 && <p className="history-more">Đang hiển thị 12 lượt mới nhất trong tổng số {events.length} lượt.</p>}</section>
  </main>;
}
